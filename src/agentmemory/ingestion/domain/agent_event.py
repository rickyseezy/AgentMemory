...
class AdapterCapabilityDescriptor:
    """Immutable version/schema/capability declaration implemented by an adapter."""
    ...
    @classmethod
    def create(  # noqa: PLR0913 -- descriptor fields are the public contract.
        cls,
        *,
        adapter_id: str,
        adapter_version: str,
        ...
        """Canonicalize unordered producer declarations into one immutable descriptor."""
        return cls(
            adapter_id=adapter_id,
            adapter_version=adapter_version,
            adapter_digest=adapter_digest,
            schema_major=schema_major,
            supported_families=tuple(sorted(set(supported_families), key=str)),
            capture_capabilities=tuple(sorted(set(capture_capabilities), key=str)),
        )

    def __post_init__(self) -> None:
        """Validate descriptor identity and internal capability consistency."""
        _require_token(self.adapter_id, "adapter_id")
        if _SEMVER.fullmatch(self.adapter_version) is None:
            msg = "adapter_version"
            raise IngestionValidationError.single(msg, "invalid_semver")
        if not _valid_digest(self.adapter_digest):
            msg = "adapter_digest"
            raise IngestionValidationError.single(msg, "invalid_digest")
        if self.schema_major != 1:
            msg = "schema_major"
            raise IngestionValidationError.single(msg, "unsupported_major")
        if not self.supported_families:
            msg = "supported_families"
            raise IngestionValidationError.single(msg, "empty")
        if not self.capture_capabilities:
            msg = "capture_capabilities"
            raise IngestionValidationError.single(msg, "empty")
        for family in self.supported_families:
            if required_capability(family) not in self.capture_capabilities:
                msg = "capture_capabilities"
                raise IngestionValidationError.single(msg, "missing_family_capability")

    @property
    def manifest_sha256(self) -> str:
        """Return the deterministic capability-manifest content identity."""
        document: JsonValue = {
            ...
            "supported_families": [item.value for item in self.supported_families],
        }
