        ...
        (
            await connection.execute(
                text(
                    ...
                    "a.sha256 AS artifact_sha256,a.byte_length,CASE "
                    "WHEN r.revoked_at IS NULL THEN policy_revoked.invalidated_at "
                    "WHEN policy_revoked.invalidated_at IS NULL THEN r.revoked_at "
                    "WHEN r.revoked_at>=policy_revoked.invalidated_at THEN r.revoked_at "
                    "ELSE policy_revoked.invalidated_at END AS revoked_at "
                    ...
                    "LEFT JOIN (SELECT policy_evidence.value AS evidence_id,"
                    "MAX(policy_invalid.invalidated_at) AS invalidated_at FROM "
                    "index_policy_derivative_invalidations AS policy_invalid,"
                    "json_each(policy_invalid.assertion_evidence_ids_json) AS policy_evidence "
                    "GROUP BY policy_evidence.value) AS policy_revoked "
                    "ON policy_revoked.evidence_id=s.evidence_id "
