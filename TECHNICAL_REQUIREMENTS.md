...
| validation | Pydantic 2.13.4 and pydantic-settings 2.14.1, strict models | canonical boundary validation and JSON Schema |
...
| container runtime | installer-provisioned Docker Engine 29.6.1 or certified Docker Desktop equivalent; Docker Compose plugin 5.1.2 | current patched single-host runtime with no user preinstallation, health-gated orchestration, volumes, networks, and profiles |
...
**Mandatory tests:** TDD unit/state-machine/property tests; kill after every Ensure/journal/config-write step; pristine no-runtime/repeated/offline/reboot-resume install; first-MCP deadline and installation-status mode; novice usability; local-model semantic smoke with packet capture; wrong runtime/image/weight signer/digest/SBOM/provenance/license; unsupported/remote Docker; stopped daemon; low disk/memory; port collision; unsafe permissions; Compose health failure; certified macOS/Linux/Windows x86_64 matrix and macOS/Linux ARM64 matrix; agent-config preservation; zero-preinstalled-runtime assertion.
...
- Maintain machine-readable SupportMatrix with owner/status/min/max versions, stock-image source, runtime-present/absent state, privilege/reboot expectations, vendor channel stability, offline redistribution status, and fixture suite.
- CI generates required matrix jobs from it; a declared supported cell cannot be manually skipped.
